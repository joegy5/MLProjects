...
class ReflexCaptureAgent(CaptureAgent):
  ...
  def getPolicyInputs(self, gameState):  
    ...
    closest_power_pellet_dist = min(map(lambda p_pellet_coord: self.compute_components_dist(agent_pos, p_pellet_coord), self.getCapsules(gameState)), key=lambda comp_dist: abs(comp_dist[0]) + abs(comp_dist[1]))
    ...
    inputs.append(agent_pos[0] / width - .5)
    ...
    for opp_idx in self.getOpponents(gameState):
      if opp_idx is not None:
        ...
        inputs.append(opp_scared_timer / 40)
  ...
  def calculate_reward(self, gameState, action):
    ...
    if new_pos < width // 2:
      reward += 10 * min(3, gameState.data.agentStates[self.index])
    ...
    reward -= 0.01
    # move towards top 3 closest foods: +0.04 for each food closer to
